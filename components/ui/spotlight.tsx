...
export function Spotlight({ className, size = 200, springOptions = { bounce: 0 } }: SpotlightProps) {
  ...
  return <motion.div
    ref={containerRef}
    aria-hidden="true"
    className={cn("pointer-events-none absolute rounded-full bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops),transparent_80%)] from-white/80 via-white/35 to-transparent blur-xl transition-opacity duration-200", isHovered ? "opacity-100" : "opacity-0", className)}
