      ...
      <CoverflowGallery
        ...
        renderSlide={(_, index, isActive) => {
          ...
          return <article className={`project-card project-card--coverflow ${isActive ? "is-front" : ""}`}>
            ...
            <div className="project-details"><div><span className="project-number">0{index + 1}</span><h3>{project.title}</h3></div><p>{project.description}</p><div className="chips">{project.languages.map((language) => <span className="chip" key={language}>{language}</span>)}</div><div className="project-actions">{project.video && <button className="project-link project-video-button icon-link" type="button" onClick={(event) => { event.stopPropagation(); setActiveVideo(project.video!); }}><HugeiconsIcon icon={PlayCircleIcon} size={15} strokeWidth={1.8} aria-hidden="true" />Watch demo</button>}{project.website && <a className="project-link project-live-link icon-link" href={project.website} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>Visit live site<HugeiconsIcon icon={ExternalLinkIcon} size={14} strokeWidth={1.8} aria-hidden="true" /></a>}</div></div>
          </article>;
        }}
      />
    </div>
